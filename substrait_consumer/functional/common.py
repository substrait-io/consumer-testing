...
def check_match(
    ...
    Calls `snapshot.assert_match(value, snapshot_name)` and returns True if the snapshot matches, returns False if it doesn't, and reraises any other exception that may occur.
    ...
    Returns:
        True if `value` matches the content of the file in `snapshot_name`, False otherwise.
    """
    try:
